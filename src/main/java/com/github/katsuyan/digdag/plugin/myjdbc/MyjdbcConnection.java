...
{
    @VisibleForTesting
    public static MyjdbcConnection open(MyjdbcConnectionConfig config)
    {
        return new MyjdbcConnection(config.openConnection());
    }

    protected MyjdbcConnection(Connection connection)
    {
        super(connection);
    }
    ...
    @Override
    public TransactionHelper getStrictTransactionHelper(String statusTableSchema, String statusTableName,
            Duration cleanupDuration) {
        return new PgPersistentTransactionHelper(statusTableSchema, statusTableName, cleanupDuration);
    }

    private class PgPersistentTransactionHelper extends AbstractPersistentTransactionHelper {
        ...
        String buildCreateTable() {
            return String.format(ENGLISH, "CREATE TABLE IF NOT EXISTS %s"
        ...
        public void cleanup() {
            executeStatement(
                    "delete old query status rows from " + escapeTableReference(statusTableReference()) + " table",
                    String.format(ENGLISH,
                            "DELETE FROM %s WHERE query_id = ANY("
                                    + "SELECT query_id FROM %s WHERE completed_at < now() - interval '%d' second" + ")",
                            escapeTableReference(statusTableReference()), escapeTableReference(statusTableReference()),
        ...
        protected StatusRow lockStatusRow(UUID queryId) throws LockConflictException {
            try (Statement stmt = connection.createStatement()) {
                ResultSet rs = stmt.executeQuery(
                        String.format(ENGLISH, "SELECT completed_at FROM %s WHERE query_id = '%s' FOR UPDATE NOWAIT",
        ...
        protected void updateStatusRowAndCommit(UUID queryId) {
            executeStatement("update status row",
                    String.format(ENGLISH, "UPDATE %s SET completed_at = CURRENT_TIMESTAMP WHERE query_id = '%s'",
                            escapeTableReference(statusTableReference()), queryId.toString()));
        ...
        protected void insertStatusRowAndCommit(UUID queryId) {
            try {
                execute(String.format(ENGLISH, "INSERT INTO %s (query_id, created_at) VALUES ('%s', CURRENT_TIMESTAMP)",
                        escapeTableReference(statusTableReference()), queryId.toString()));
